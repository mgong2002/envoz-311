...
(function () {
  ...
  function confChipsHTML(pct, rationale, source) {
    ...
    if (source) html += ' <span class="chip chip-source">📄 ' + esc(source) + "</span>";
  ...
  function caseCardHTML(opts) {
    ...
    return (
      ...
      '<h4 style="margin:8px 0 2px">' + esc(opts.type) + "</h4>" +
      '<div class="small" style="color:var(--muted)">' + esc(opts.location) + " · " + esc(opts.dept) + "</div>" +
      (opts.proof ? '<div class="proof-strip" style="margin-top:10px"><span class="proof-icon">✓</span>' + esc(opts.proof) + "</div>" : "") +
      (opts.extra || "") +
      "</div>"
    );
  }
  ...
  function statusModal(caseId, type, doneIdx, extraNote) {
    ...
    window.Envoz.openModal(
      ...
      '<div class="caution-strip"><span>🕰</span><span>Envoz never invents repair dates. This view shows only what the CRM has actually confirmed' +
  ...
  var ACTIONS = {
    "photo-4281": function () {
      window.Envoz.openModal(
        ...
        '<div class="proof-strip" style="margin-top:16px"><span class="proof-icon">🔒</span>Photos are cropped, and faces and plates blurred, before any public display. Originals stay tenant-scoped.</div>'
      );
    },
    ...
    "photo-4293": function () {
      window.Envoz.openModal(
        ...
        '<div class="proof-strip"><span class="proof-icon">🔒</span>Cropped and blurred before public display — see Bad-day behaviors below.</div>'
      );
    },
    ...
    "new-instead": function () {
      window.Envoz.openModal(
        ...
        '<div class="caution-strip"><span>🔁</span><span>Linked-duplicate handling keeps queues clean during surges without hiding any resident’s report.</span></div>' +
    ...
    "text-routed-4320": function () {
      smsModal("Text me when it’s routed", [
        ["in", "Thanks for reporting the beehive at Jeffrey Transit Stop. A 311 specialist is reviewing it now (target: 4 hours)."],
        ["in", "We’ll text you the case number the moment it’s routed to the right team."]
      ], "Unknowns are never forced into a wrong category just to close the call.");
    },
    "why-no-case": function () {
      window.Envoz.openModal(
        ...
        '<div class="safety-strip"><span>⛑</span><span>Red is reserved for emergencies across the entire product — it always means 911, never a queue.</span></div>' +
    ...
    "city-owned-after-all": function () {
      window.Envoz.openModal(
        ...
        "<li>Staff corrections to ownership boundaries feed the learning ledger, so the GIS jurisdiction layer improves.</li>" +
        "<li>The resident keeps one thread the whole way — no dead ends, no “call someone else.”</li></ul>" +
        '<div style="margin-top:14px" class="flex flex-wrap"><a class="btn btn-primary btn-sm" href="dashboards.html?tab=cel">See the learning ledger</a><button class="btn btn-secondary btn-sm" type="button" onclick="Envoz.closeModal()">Close</button></div>'
      );
    }
  };
  ...
  var SCENARIOS = [
    ...
    (function () {
      ...
      return {
        id: "pothole", pill: "🕳 Pothole / road hazard",
        ...
        events: [
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no injuries or immediate danger reported." } },
          { n: "Glad you're safe. Is this closer to Irvine Center Drive or Mesa Ridge?" },
          { u: "Irvine Center Drive, right by the ramp." },
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — " + c.location + " · GIS road ownership layer." }, d: 500 },
          { step: { ic: "🔁", cls: "ok", html: "<strong>Duplicate check</strong> — 1 related: " + c.related[0] + ". Not a duplicate." } },
          { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — city-maintained roadway → " + c.dept + "." } },
          ...
          { actions: [
            { label: "📷 Add photo when safe", act: "photo-4281" },
            { label: "🔔 Track status", act: "track-4281", primary: true }
          ], d: 700 },
    ...
    (function () {
      ...
      return {
        id: "dumping", pill: "🛋 Illegal dumping / bulk waste",
        ...
        events: [
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no hazard to people; standard bulk-waste removal." } },
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — " + c.location + " · alley alias matched to public right-of-way." }, d: 500 },
          { step: { ic: "🔁", cls: "warn", html: "<strong>Duplicate check</strong> — 2 prior dumping cases in this alley (VR-4201, VR-4144). New incident — repeat-location pattern flagged for Sanitation." } },
          { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — public right-of-way → " + c.dept + "." } },
          { chips: { pct: c.confidence, rat: c.rationale } },
          ...
          { outcome: caseCardHTML({ id: c.id, type: c.type, location: c.location, dept: c.dept, priority: "Medium", proof: c.proof }) },
          { actions: [
            { label: "📷 Add photo", act: "photo-4293" },
            { label: "🔔 Track status", act: "track-4293", primary: true }
          ], d: 700 },
    ...
    /* 3 — Water leak (dedupe → follow vs new) */
    {
      ...
      events: [
        ...
        { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no flooding of structures or roadway." } },
        { n: "Got it. Is it near the corner with Canyon Way, by the storm drain?" },
        { u: "Yes, right by the drain grate." },
        { pin: true },
        { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — Oak Bluff Dr near storm drain #SD-2214 · GIS storm-drain asset layer." }, d: 500 },
        { step: { ic: "🔁", cls: "warn", html: "<strong>Duplicate check</strong> — open case <strong>VR-4317 · Water Pooling</strong>, same block, reported 8:20 PM. Likely the same water." } },
        ...
        { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — Utilities, per the storm-drain proximity rule city staff approved in June." } },
        ...
        { outcome:
          ...
          '<h4 style="margin:8px 0 2px">Water Pooling — existing case</h4>' +
          '<div class="small" style="color:var(--muted)">Oak Bluff Dr near storm drain #SD-2214 · Utilities</div>' +
          '<div class="proof-strip" style="margin-top:10px"><span class="proof-icon">✓</span>Added as follower to VR-4317 · SMS status link sent at 9:03 PM</div></div>' },
        { actions: [
          { label: "💬 Text me updates on VR-4317", act: "sms-follow-4317", primary: true },
          { label: "Report as new instead", act: "new-instead" }
        ], d: 700 },
        { audit: 'CEL event logged · <span class="mono">DUPLICATE_FOLLOWED</span> · VR-4317 · 21:03 PT', d: 700 }
      ]
    },
    ...
    (function () {
      ...
      return {
        ...
        events: [
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no wires involved, no one trapped or injured." } },
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — " + c.location + " · parks asset layer." }, d: 500 },
          { step: { ic: "🔁", cls: "warn", html: "<strong>Duplicate check</strong> — related: " + c.related[0] + ". Different asset, likely the same windstorm — new case, linked." } },
          { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — inside park boundary → " + c.dept + "." } },
          { chips: { pct: c.confidence, rat: c.rationale } },
          ...
          { outcome: caseCardHTML({ id: c.id, type: c.type, location: c.location, dept: c.dept, priority: "High", proof: c.proof }) },
          { actions: [
            { label: "🔔 Track status", act: "track-4314", primary: true },
    ...
    (function () {
      ...
      return {
        id: "streetlight", pill: "🌐 Streetlight · Spanish call",
        ...
        events: [
          ...
          { step: { ic: "🌐", cls: "lang", html: "<strong>Language detected — Español</strong> · 94% language confidence. Conversation continues in Spanish." }, d: 600 },
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no immediate danger; visibility concern noted." } },
          ...
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — pole TL-0921 at Maple &amp; 3rd · streetlight asset layer." }, d: 500 },
          { step: { ic: "🔁", cls: "ok", html: "<strong>Duplicate check</strong> — no open cases on this pole." } },
          { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — city-owned luminaire → " + c.dept + " · <strong>school-zone flag raises priority</strong>." } },
          { chips: { pct: c.confidence, rat: c.rationale } },
          ...
          { outcome: caseCardHTML({ id: c.id, type: c.type, location: c.location, dept: c.dept, priority: "Medium", proof: c.proof,
            extra: '<div class="chip-row" style="margin-top:10px"><span class="chip chip-purple">🌐 Español · 94% language confidence</span><span class="chip chip-amber">School-zone priority</span></div>' }) },
          { actions: [
            { label: "💬 Recibir estado por SMS", act: "sms-es-4330", primary: true },
            { label: "🔔 Track status", act: "track-4330" }
          ], d: 700 },
    ...
    (function () {
      ...
      return {
        id: "storm", pill: "🌧 Storm drain backup · after-hours",
        ...
        events: [
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no water in structures; monitoring guidance given." } },
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — " + c.location + "." }, d: 500 },
          { step: { ic: "🌀", cls: "warn", html: "<strong>Surge cluster</strong> — 14 similar “storm grate burbling” reports within 0.5 mi in 48 hours. Likely one storm event." } },
          { step: { ic: "🗺", cls: "warn", html: "<strong>Jurisdiction</strong> — Public Works (provisional). Phrase is below the confident-routing threshold — this is a new pattern, not a known category." } },
          { chips: { pct: c.confidence, rat: c.rationale } },
          ...
          { outcome: caseCardHTML({ id: c.id, type: "Storm Drain Backup (provisional)", location: c.location, dept: "Public Works · staff review", priority: "Medium", proof: c.proof,
            extra: '<div class="caution-strip" style="margin-top:10px"><span>◔</span><span>64% confidence — held for staff review with transcript and 14-report cluster context. No guess was forced.</span></div>' }) },
          { actions: [
            { label: "🔔 Track status", act: "track-4342", primary: true },
            { label: "🌀 See the surge cluster", href: "dashboards.html?tab=performance&mode=surge" }
          ], d: 700 },
          { audit: 'CEL event logged · <span class="mono">SERVICE_REQUEST_CREATED</span> + <span class="mono">NEEDS_REVIEW</span> · VR-4342 · 22:33 PT · cluster n=14', d: 700 }
        ]
      };
    })(),
    ...
    (function () {
      ...
      return {
        id: "beehive", pill: "🐝 Unknown · beehive in bus shelter",
        title: "Unknown request · beehive in bus shelter", time: "3:45 PM",
        meta: [["chip-ocean", "Voice call"], ["chip-amber", "Learning queue"]],
        map: {
        ...
        events: [
          ...
          { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — no active stings or allergic reactions." } },
          { pin: true },
          { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — " + c.location + " · transit asset layer." }, d: 500 },
          { step: { ic: "🔁", cls: "ok", html: "<strong>Duplicate check</strong> — no similar open reports." } },
          { step: { ic: "🗺", cls: "warn", html: "<strong>No confident taxonomy match</strong> — candidates: Parks Maintenance 41% · Code Enforcement 33% · Animal / Vector Control 29%. All below the routing threshold." } },
          { chips: { pct: 41, rat: "Below routing threshold — not forced into a wrong category." } },
          { n: "Honestly, this one doesn't fit a category I'm confident about — and I'd rather not guess. A 311 specialist will review it within 4 hours, and I'll text you the moment it's routed." },
          { outcome:
            '<div class="card card-pad-sm">' +
            '<h4 style="margin:0 0 4px">Held in the learning queue</h4>' +
            '<div class="small" style="color:var(--muted)">Human review within 4 hours · no CRM case forced yet</div>' +
            '<div class="caution-strip" style="margin-top:10px"><span>◔</span><span>41% confidence — the request goes to a person, not into the wrong department’s queue.</span></div>' +
            '<p class="small" style="margin:10px 0 0;color:var(--muted)">Unknowns teach the system: if staff approve it, “hive/nest in city asset” becomes a real category with a real owner.</p></div>' },
          { actions: [
            { label: "🗂 See it in staff triage", href: "staff-triage.html", primary: true },
            { label: "💬 Text me when it's routed", act: "text-routed-4320" }
          ], d: 700 },
          { audit: 'CEL event logged · <span class="mono">HELD_FOR_REVIEW</span> · learning queue · 15:45 PT · candidates=3', d: 700 }
        ]
      };
    })(),
    ...
    {
      id: "emergency", pill: "⛑ Emergency misdial · downed wire",
      ...
      events: [
        ...
        { step: { ic: "⛑", cls: "alert", html: "<strong>Safety gate — life-safety hazard detected:</strong> downed, energized wire. Routine intake halted." }, d: 700 },
        ...
        { step: { ic: "⏸", cls: "alert", html: "<strong>Pipeline halted</strong> — location, duplicate, and routing steps are suspended during emergencies." } },
        { outcome:
          '<div class="safety-strip"><span>⛑</span><span><strong>Downed wire → 911 guidance given.</strong> No ordinary 311 case was created — emergencies are never queued behind potholes, and no case number was fabricated.</span></div>' +
    ...
    {
      id: "trash", pill: "🗑 Trash day lookup",
      ...
      events: [
        ...
        { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate</strong> — informational request; no safety screening needed." }, d: 600 },
        { n: "Happy to check. What street or neighborhood are you in?" },
        { u: "Oak Bluff, on Canyon Way." },
        { pin: true },
        { step: { ic: "📍", cls: "info", html: "<strong>Neighborhood resolved</strong> — Oak Bluff · Sanitation service-area layer." }, d: 500 },
        { step: { ic: "📚", cls: "ok", html: "<strong>Grounded lookup</strong> — answer drawn from the city trash schedule, not from guesswork. No case needed." } },
        { step: { ic: "🗺", cls: "ok", html: "<strong>Jurisdiction</strong> — city Sanitation service area." } },
        ...
        { outcome:
          '<div class="card card-pad-sm">' +
          '<h4 style="margin:0 0 4px">Grounded answer — no case needed</h4>' +
          ...
          '<div class="chip-row"><span class="chip chip-source">📄 City trash schedule · updated Jun 2026</span></div>' +
          '<div class="proof-strip" style="margin-top:10px"><span class="proof-icon">✓</span>Schedule texted to the caller at 10:05 AM</div></div>' },
        { actions: [
          { label: "💬 Text me the schedule", act: "sms-schedule", primary: true },
          { label: "🎉 Holiday shift reminders", act: "holiday-reminders" }
        ], d: 700 },
        { audit: 'CEL event logged · <span class="mono">ANSWERED_FROM_KNOWLEDGE</span> · source: trash schedule (Jun 2026) · 10:05 PT', d: 700 }
      ]
    },

    /* 10 — External ownership referral */
    {
      id: "referral", pill: "🛣 State-owned asset · warm referral",
      ...
      events: [
        ...
        { step: { ic: "🛟", cls: "ok", html: "<strong>Safety gate cleared</strong> — congestion, not an immediate hazard." } },
        { pin: true },
        { step: { ic: "📍", cls: "info", html: "<strong>Location resolved</strong> — I-5 NB on-ramp at Alton Pkwy." }, d: 500 },
        { step: { ic: "🔁", cls: "ok", html: "<strong>Duplicate check</strong> — no open city or referral records for this ramp." } },
        { step: { ic: "🗺", cls: "warn", html: "<strong>Jurisdiction</strong> — GIS ownership layer shows the ramp meter is a <strong>Caltrans District 12</strong> asset, not city-maintained." } },
        ...
        { outcome:
          '<div class="card card-pad-sm">' +
          '<h4 style="margin:0 0 4px">Warm referral → Caltrans District 12</h4>' +
          '<div class="small" style="color:var(--muted)">I-5 NB on-ramp at Alton Pkwy · ramp meter fault</div>' +
          '<div class="proof-strip" style="margin-top:10px"><span class="proof-icon">✓</span>Referral sent with location + transcript summary at 4:12 PM · reference VR-REF-1188</div>' +
          '<p class="small" style="margin:10px 0 0;color:var(--muted)">No dead ends: the referral is logged in the city’s ledger, and if Caltrans bounces it back it converts to a city case with the original call attached.</p></div>' },
        { actions: [
          { label: "💬 Text me the Caltrans link", act: "sms-caltrans", primary: true },
          { label: "What if it's city-owned after all?", act: "city-owned-after-all" }
        ], d: 700 },
        { audit: 'CEL event logged · <span class="mono">REFERRED_EXTERNAL</span> · Caltrans D12 · VR-REF-1188 · 16:12 PT', d: 700 }
      ]
    }
  ];
  ...
  function addStep(s) {
    ...
    el.innerHTML = '<span class="ic" aria-hidden="true">' + s.ic + "</span><span>" + s.html + "</span>";
  ...
  function runEvent(ev) {
    ...
    if (ev.audit) {
      els.audit.innerHTML = '<div class="audit-ribbon"><span aria-hidden="true">🛡</span><span>' + ev.audit + "</span></div>";
      return;
    }
  }
